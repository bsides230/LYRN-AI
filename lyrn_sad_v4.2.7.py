...
import webbrowser
from modules.rwi_builder.server import RWIServer
...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def __init__(self, master, log_queue: queue.Queue):
        ...
        self.rwi_server = RWIServer(self, SCRIPT_DIR)
        self.rwi_server_port = self.rwi_server.start()
    ...
    def open_prompt_builder(self):
        """Opens the prompt builder in the web browser."""
        url = f"http://localhost:{self.rwi_server_port}/index.html"
        webbrowser.open(url)
        self.update_status("Opened System Prompt Builder in browser.", LYRN_INFO)
