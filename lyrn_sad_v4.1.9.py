...
class StreamHandler:
    ...
    def _process_standard_content(self, content: str):
        ...
        thinking_starts = ['<thinking>', '<think>', '<analysis>']
        thinking_ends = ['</thinking>', '</think>', '</analysis>']
        if any(tag in content for tag in thinking_starts) or self.thinking_content:
            for start_tag, end_tag in zip(thinking_starts, thinking_ends):
                if start_tag in content:
                    self.current_think_end = end_tag
                    self.thinking_content = content.split(start_tag)[-1]
                    content = content.split(start_tag)[0]
                    break
            else:
                if self.current_think_end and self.current_think_end in content:
                    thinking_end = content.split(self.current_think_end)[0]
                    self.thinking_content += thinking_end
                    content = content.split(self.current_think_end, 1)[-1]
                    ...
                    content = ""
        if content:
            self.gui_queue.put(('token', content, self.current_role))
...
class TabbedSettingsDialog(ThemedPopup):
    ...
    def create_ui_settings_tab(self):
        ...
        self.show_reasoning_var = ctk.BooleanVar(value=self.settings_manager.ui_settings.get("show_reasoning", True))
        reasoning_checkbox = ctk.CTkCheckBox(display_frame, text="Show thinking/analysis text", variable=self.show_reasoning_var)
        reasoning_checkbox.pack(anchor="w", padx=10, pady=5)
    ...
    def load_current_settings(self):
        ...
        self.show_reasoning_var.set(self.settings_manager.ui_settings.get("show_reasoning", True))
    ...
    def save_all_settings(self):
        ...
        try:
            ...
            self.settings_manager.ui_settings["show_reasoning"] = self.show_reasoning_var.get()
...
class ThemeBuilderPopup(ThemedPopup):
    ...
    def create_theme_builder_widgets(self):
        ...
        color_labels = {
            ...
            "assistant_text": "Assistant Text", "thinking_text": "Thinking Text", "analysis_text": "Analysis Text",
...
class LyrnAIInterface(ctk.CTkToplevel):
    ...
    def create_chat_area(self):
        ...
        self.analysis_display.tag_config("analysis_text", foreground=self.theme_manager.get_color("analysis_text"))
...
"""
    ...
    def apply_color_theme(self):
        ...
        try:
            ...
            if hasattr(self, 'chat_display'):
                ...
                self.chat_display.tag_config("system_text", foreground=tm.get_color("system_text"))
                self.chat_display.tag_config("user_text", foreground=tm.get_color("user_text"))
                self.chat_display.tag_config("assistant_text", foreground=tm.get_color("assistant_text"))
                self.chat_display.tag_config("thinking_text", foreground=tm.get_color("thinking_text"))
                self.chat_display.tag_config("error", foreground=tm.get_color("error"))
                self.chat_display.tag_config("success", foreground=tm.get_color("success"))
            if hasattr(self, 'analysis_display'):
                self.analysis_display.tag_config("analysis_text", foreground=tm.get_color("analysis_text"))
    ...
    def send_message(self):
        ...
        if self.settings_manager.ui_settings.get("show_reasoning", True):
            self.display_colored_message("Assistant: Thinking...\n\n", "thinking_text")
        self.is_thinking = True

        # Show analysis display if it's an OSS model and reasoning display is enabled
        if (
            self.settings_manager.settings.get("active", {}).get("is_oss_model", False)
            and self.settings_manager.ui_settings.get("show_reasoning", True)
        ):
            ...
            while True:
                try:
                    ...
                    if message[0] == 'token':
                        ...
                        tag = 'assistant_text' if msg_type in ('assistant', 'model') else f"{msg_type}_text"
                        if not hasattr(self, '_assistant_started'):
                            label = "Assistant: " if msg_type != 'model' else "Model: "
                            self.display_colored_message(label, 'assistant_text')
                        ...
                        if self.settings_manager.ui_settings.get('show_reasoning', True):
                            ...
                            self.analysis_display.insert("end", content, "analysis_text")
