...
MODEL_CHANNEL_MARKERS = {
    "gemma": {
        "analysis_start": "<think>",
        "final_start": "<response>"
    },
    "qwen": {
        "analysis_start": "<|im_start|>analysis<|im_end|>",
        "final_start": "<|im_start|>{role}<|im_end|>"
    },
    "openai_oss": {
        "analysis_start": "<|channel|>analysis<|message|>",
        "final_start": "<|start|>{role}<|channel|>final<|message|>"
    },
    "smolml3": {
...
class StreamHandler:
    ...
    def __init__(self, gui_queue, metrics: EnhancedPerformanceMetrics, is_oss_model: bool = False, model_metadata: Optional[Dict[str, Any]] = None):
        self.gui_queue = gui_queue
        self.metrics = metrics
        self.is_oss_model = is_oss_model
        self.model_metadata = model_metadata or {}
        self.default_role = self.model_metadata.get("role", "assistant")
        model_key = self.model_metadata.get("chat_format")
        if self.is_oss_model and not model_key:
            model_key = "openai_oss"
        self.channel_markers = MODEL_CHANNEL_MARKERS.get(model_key, MODEL_CHANNEL_MARKERS.get("openai_oss"))
    ...
    def handle_token(self, token_data):
        ...
        if 'choices' in token_data and len(token_data['choices']) > 0:
            ...
            role = delta.get('role') or self.default_role

            if content:
                if self.is_oss_model:
                    self.buffer += content
                    self._process_oss_buffer(role)
                else:
                    # Original logic for non-OSS models
                    self._process_standard_content(content, role)
    ...
    def _process_standard_content(self, content: str, role: str):
        ...
        if '<thinking>' in content or self.thinking_content:
            if '<thinking>' in content:
                ...
                content = ""
        if content:
            self.gui_queue.put(('token', content, role))

    def _process_oss_buffer(self, role: str):
        """Processes the buffer for special OSS channel markers."""
        analysis_start_tag = self.channel_markers.get("analysis_start", "")
        final_start_template = self.channel_markers.get("final_start", "")
        final_start_tag = final_start_template.format(role=role)
        ...
        if self.in_analysis_channel:
            ...
            if self.buffer:
                ...
                self.gui_queue.put(('token', self.buffer, role))
                self.buffer = ""
...
"""
    ...
    def _execute_reflection_job(self, job: Job):
        ...
        try:
            ...
            active = self.settings_manager.settings["active"]
            handler = StreamHandler(self.stream_queue, self.metrics, model_metadata=active)
    ...
    def generate_response(self, user_text: str):
        ...
        try:
            ...
            handler = StreamHandler(self.stream_queue, self.metrics, is_oss_model=is_oss_model, model_metadata=active)
