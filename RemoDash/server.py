...
# Logic to detect if we should run in current directory or chdir to 'web'
# This supports two structures:
# 1. Root/settings.json + Root/web/dashboard.html (Standard/RemoDash) -> Stay in Root
# 2. Root/web/settings.json + Root/web/dashboard.html (LYRN) -> Enter Web
target_dir = "."
if os.path.exists("settings.json"):
    target_dir = "."
elif os.path.exists("web/settings.json"):
    target_dir = "web"
elif os.path.exists("web/automation") or os.path.exists("web/chat"):
    # Fallback for fresh LYRN install (settings might be missing but app data exists)
    target_dir = "web"

if target_dir != ".":
    print(f"[System] Changing working directory to: {target_dir}")
    os.chdir(target_dir)
...
# Static Files
# Detect where dashboard.html is (relative to the now potentially changed CWD)
static_dir = "."
